...
export default function PantallaPrincipalVigilante() {
        ...
        if (token) {
          try {
            await fetch('http://192.168.0.103:3000/registrarPushToken', {
              ...
              body: JSON.stringify({ token, idUsuario: idVigilante, plataforma: Platform.OS }),
            });
          } catch {}
        }
      })
  ...
  const FuncionAgregarAnuncioVigilante = async () => {
  ...
  const handleFabPress = async () => {
    ...
    await FuncionAgregarAnuncioVigilante();
  };
  ...
  <FAB icon="alert" style={styles.fab} size="large" onPress={handleFabPress} color="white" />
          ...
          <Modal visible={modalAgregarVisible} onDismiss={() => setModalAgregarVisible(false)}>
            <View style={styles.modalView}>
              ...
              <View style={styles.modalBtns}>
                ...
                <Button mode="contained" onPress={FuncionAgregarAnuncioVigilante} loading={enviando}>
                  Agregar
                </Button>
              </View>
            </View>
...
const styles = StyleSheet.create({
  ...
  fab: { margin: 16, backgroundColor: '#e34040ff', alignSelf: 'center', position: 'relative', elevation: 4 },
  modalView: { backgroundColor: '#fff', margin: 20, borderRadius: 10, padding: 20, alignItems: 'center', shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, shadowRadius: 4, elevation: 5 },
  modalTitle: { fontSize: 20, fontWeight: 'bold', marginBottom: 15, color: '#333', textAlign: 'center' },
  input: { width: 280, minHeight: 80, borderColor: '#ccc', borderWidth: 1, borderRadius: 8, paddingHorizontal: 10, paddingVertical: 8, marginBottom: 15, backgroundColor: '#fafafa' },
