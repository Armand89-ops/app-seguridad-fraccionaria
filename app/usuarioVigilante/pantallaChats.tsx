...
import { ActivityIndicator, FlatList, Keyboard, Platform, SafeAreaView, StatusBar, StyleSheet, Text, TextInput, TouchableOpacity, View } from 'react-native';
...
type Mensaje = { _id?: string; idChat: any; idUsuario: any; contenido: string; fechaEnvio: string };

export default function PantallaChatsVigilante() {
  ...
  const extraOffset = 45; // píxeles extra por encima del teclado
  const navBarInset = Platform.OS === 'android' ? 0 : insets.bottom;
  ...
  const idYo = useMemo(() => normalizeId(idVigilante), [idVigilante]);

  // Cargar datos base (usuarios, edificio del vigilante) y chats visibles
  useEffect(() => {
    let cancel = false;
    async function cargar() {
      try {
        ...
        const yo = Array.isArray(usuariosResp) ? usuariosResp.find((u: any) => normalizeId(u._id) === idYo) : null;
    ...
    return () => { s.disconnect(); cancel = true; };
  }, [idYo]);
  ...
  const enviarMensaje = async () => {
    ...
    try {
      await fetch(`${baseUrl}/agregarMensaje`, {
        ...
        body: JSON.stringify({ idChat: chatSeleccionado._id, idUsuario: idYo, contenido })
      });
    } catch {
      // En caso de error puedes restaurar el texto para reintentar
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.chatScreen}>
          ...
          <View style={styles.headerBar}>
            <TouchableOpacity onPress={() => { setChatSeleccionado(null); setMensajes([]); }} style={styles.backBtn}>
          ...
          <FlatList
            ...
            onContentSizeChange={() => {
              // Cada vez que se manda un nuevo mensaje auto scroll al final
              listaRef.current?.scrollToEnd({ animated: true });
            }}
...
const styles = StyleSheet.create({
  ...
  headerBar: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 10, backgroundColor: '#ffffff', borderBottomWidth: StyleSheet.hairlineWidth, borderColor: '#ddd' },
  ...
  navBarSpacer: { position: 'absolute', left: 0, right: 0, bottom: 0, backgroundColor: '#000', zIndex: 1 },
